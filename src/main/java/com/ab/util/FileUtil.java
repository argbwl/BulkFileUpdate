...
import com.ab.test.Test;

public class FileUtil {

	//to read file from class path or resources folder
	public static File getFileFromClassPath(String fileName) {
		ClassLoader classLoader = new Test().getClass().getClassLoader();
		return new File(classLoader.getResource(fileName).getFile());
	}

	public static void writeToFile(int startNo, int endNo, File file) throws IOException {
		if (file == null)
	...
	public static List<Bank> getBankListFromFile(File file) throws IOException{
		...
		try(BufferedReader reader = new BufferedReader(new FileReader(file))){
			...
			while((line=reader.readLine())!=null) {
				...
				bank.setEstablishedYear(Integer.parseInt(dataArr[3]));
				bankList.add(bank);
			}
		}
		return bankList;
